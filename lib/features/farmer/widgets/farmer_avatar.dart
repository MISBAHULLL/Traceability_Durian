...
// [FE - Component Rendering] FarmerAvatar menampilkan foto profil petani
...
/// Avatar profil petani — foto atau inisial, dengan opsi edit.
class FarmerAvatar extends StatelessWidget {
  ...
  final dynamic profile; // FarmerProfile — pakai dynamic agar tidak circular
  ...
  Widget build(BuildContext context) {
    ...
    final String? avatarPath = profile.avatarPath as String?;
    final hasPhoto = avatarPath != null && avatarPath.isNotEmpty;

    Widget avatar;
    if (hasPhoto) {
      avatar = _photoAvatar(avatarPath, size);
    } else {
      avatar = _initialsAvatar(fullName, size);
    }
  ...
  // [FE - Event Handler] _pickAvatar membuka bottom sheet pilihan sumber
  // foto, lalu menyimpan path ke repository via updateAvatar.
        ...
        child: Column(
          ...
          children: [
            ...
            ListTile(
              ...
              onTap: () =>
                  Navigator.pop(sheetCtx, _AvatarAction.camera),
            ),
            ListTile(
              ...
              onTap: () =>
                  Navigator.pop(sheetCtx, _AvatarAction.gallery),
            ),
            if (hasPhoto)
              ListTile(
                ...
                onTap: () =>
                    Navigator.pop(sheetCtx, _AvatarAction.remove),
              ),
            const SizedBox(height: 8),
          ],
        ),
